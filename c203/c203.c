...
void Queue_Init(Queue *queue)
{
	...
	{ // všechny hodnoty ve frontě nastaví na '*'
		queue->array[i] = '*';
	}
...
{
	return ((index + 1) % QUEUE_SIZE); // pokud index přesáhne QUEUE_SIZE, tak vrátí index prvního prvku
}
...
{
	return (queue->firstIndex == queue->freeIndex); // fronta je prázdná, pokud začátek a konec fronty je na stejném indexu
}
...
{
	return nextIndex(queue->freeIndex) == queue->firstIndex; // fronta je plná, pokud je zaplněno QUEUE_SIZE - 1 prvků fronty
}
...
void Queue_Front(const Queue *queue, char *dataPtr)
{
	...
	*dataPtr = queue->array[queue->firstIndex]; // vrátí hodnotu z prvního prvku fronty
}
...
void Queue_Remove(Queue *queue)
{
	...
	queue->firstIndex = nextIndex(queue->firstIndex); // posune ptr prvního prvku a tím odstraní prvek ze začátku fronty
}
...
void Queue_Enqueue(Queue *queue, char data)
{
	...
	queue->array[queue->freeIndex] = data;			// vloží znak do fronty
	queue->freeIndex = nextIndex(queue->freeIndex); // posune index konce fronty
}
